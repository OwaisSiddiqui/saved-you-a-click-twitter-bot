...
const extractUrls = (str: string, lower = false) => {
  const regexp =
...
const getArticleDataUsingFetch = async (url: string): Promise<ArticleData> => {
...
const getArticleDataUsingPuppeteer = async (
  tweetUrl: TweetUrl
): Promise<ArticleData> => {
...
const getMentionsUsers = async (client, id) => {
  ...
  const tweetUrls: TweetUrl[] = [];

  if (!allMentionUsers) {
    throw new Error("no mentions");
  }

  const mentionsUsers: any[] = [];
  for (let mention of allMentionUsers) {
    ...
    if (
      originalTweet.data.find(
        (replyTweet) =>
          replyTweet.author_id === id &&
          replyTweet.in_reply_to_user_id === mention.author_id
      ) === undefined
    ) {
...
const getParentTweet = async (client: Client, tweet: Tweet) => {
...
const replyWithAnswer = async (client: Client, tweet: Tweet) => {
  ...
  const question = tweetText.substring(
    tweetText.lastIndexOf("@saveaclickbot") + "@saveaclickbot".length
  );
...
const streamConnect = async (appClient: Client, botClient: Client) => {
